...
%% NEW CODE FOR PLATE WITH HOLE 
% constrain inner boundary to have same displcaement
numFreeDof = numFreeDof + 1;
jtDofNum(inbnd) = numFreeDof;
...
%% Load vector - SCRAP THIS - not used in this problem
...
for m = 1:numMem
    ...
    [Kmem,Pmem]=getTriangleStiffMatAndLoadVector(coord(connect(m,:),:), ...
                                                     pPress, STens);
    ...
    Pfree(indexActiveDof) = Pfree(indexActiveDof) +...
        Pmem(maskActiveDof);
end
...
defscale = 10;
figure,
    patch('faces',connect(:,[1 2 3 1]),'vertices',[coord u],...
          'facecolor','r')
axis equal
